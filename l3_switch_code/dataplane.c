...
void table_ipv4_fib_lpm_key(packet_descriptor_t* pd, uint8_t* key) {
	//printf("    :: extracting dstAddr\n");
	EXTRACT_INT32_BITS(pd, field_instance_ipv4_dstAddr, *(uint32_t*)key)	
	/*
	printf("    :: extracted dstAddr - the key is ");  print_key(key,4);
 ...
 {// sugar@69
     printf("  :::: EXECUTING TABLE ipv4_fib_lpm\n");
     uint8_t* key[4];
     ...
     //printf("The egress port is %d\n",*(res->fib_hit_nexthop_params.port));
     ...
     if(res == NULL) {
       ...
       switch (res->action_id) {
         case action_on_miss:
           printf("    :: EXECUTING ACTION on_miss...\n");
           action_code_on_miss(pd, tables);
           break;
         case action_fib_hit_nexthop:
           printf("    :: EXECUTING ACTION fib_hit_nexthop...\n");
 ...
 {
     printf("  :::: EXECUTING TABLE sendout\n");
     uint8_t* key[2];
     ...
     if(res == NULL) {
       ...
       switch (res->action_id) {
         ...
         case action_rewrite_src_mac:
           printf("    :: EXECUTING ACTION rewrite_src_mac...\n");
...
void reset_headers(packet_descriptor_t* packet_desc) {// sugar@229
 ...
 int verify_packet(packet_descriptor_t* pd) {
   uint32_t value32;
   if(pd->headers[header_instance_ipv4].pointer != NULL)
   {
     EXTRACT_INT32_BITS(pd, field_instance_ipv4_hdrChecksum, value32);
     if(value32 != calculate_ipv4_checksum(pd)) {
       printf("Checksum verification on field 'ipv4.hdrChecksum' by 'ipv4_checksum'.\n");
       return 1;
     }
     else printf("Checksum verification on field 'ipv4.hdrChecksum' by 'ipv4_checksum'\n");
   }
   return 0;
 }
